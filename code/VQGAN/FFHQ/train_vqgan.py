...
def main_worker(args):
    ...
    model_para = list(model.module.encoder.parameters()) + list(model.module.decoder.parameters()) + list(model.module.pre_quant_proj.parameters()) + list(model.module.post_quant_proj.parameters())
    ...
    optimizer = torch.optim.AdamW([{'params': model_para}, {'params': code_para, 'lr': 0.02}], lr=args.lr, betas=(0.9, 0.95))
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for step, (x, _) in enumerate(train_dataloader):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float32):
                ...
                gen_loss, gen_loss_pack = model.module.vqgan(x, x_rec, vq_loss, global_epoch=epoch, optimizer_idx=0, last_layer=model.module.decoder.last_layer)
                gen_loss.backward()
                has_nan = False            
                ...
                disc_optimizer.zero_grad()
                d_loss, d_loss_pack = model.module.vqgan(x, x_rec, vq_loss, global_epoch=epoch, optimizer_idx=1)
